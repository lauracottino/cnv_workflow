...
include { check_files; splitSampleBams lumpy } from './modules/data
include { lumpy_preProcess; run_lumpy; lumpy_Postprocess; lumpy_mergeSampleChr } from './modules/cnv_calling'_prep'

// TODO create container
// TODO create config file to separate configs 

// INPUTS
// ref_hg19  = file("/home/phelelani/projects/laura/data/indexes/hg19/human_g1k_v37_decoy.fa", type:'file')
ref_hg38  = file("/spaces/gerrit/dbs/gatk/bundle/hg38/Homo_sapiens_assembly38.fasta", type:'file')
bams = Channel.fromFilePairs("/external/diskC/build38/datasets/1000G/bam/NA18488{.bam,.bam.bai}", size:-2)
...
workflow data_preparation{
    ...
    main:
        splitSampleBams(data)

    emit: 
        splitSample = splitSampleBams

workflow lumpy{
    take: split_bams

    main:
        lumpy_preProcess(split_bams)
...
workflow {
    // Data preparation
    check_files([ref_hg38])
    data_preparation(bams.combine(ch
    lumpy(data_preparation.out.splitSample)
roms))
}
// // =============================================== SPLIT ============================================== //
